...
namespace Cdy.Tag
{
    ...
    public class QuerySerivce: IHisQuery
    {
        ...
        {
            var vfiles = GetFileManager().GetDataFiles(times, id);
        ...
        {
            var vfiles = GetFileManager().GetDataFiles(startTime, endTime,id);
            vfiles.ForEach(e => {
        ...
        public HisQueryResult<T> ReadAllValue<T>(int id, DateTime startTime, DateTime endTime)
        {
            ...
            else if (typeof(T) == typeof(byte))
            {
                ...
                var result = new HisQueryResult<byte>(valueCount);
                ReadAllValue(id, startTime, endTime, result);
                return result as HisQueryResult<T>;
            }
            else if (typeof(T) == typeof(string) || typeof(T) == typeof(String))
            {
                var result = new HisQueryResult<string>(valueCount);
                ReadAllValue(id, startTime, endTime, result);
                return result as HisQueryResult<T>;
            }
            else if (typeof(T) == typeof(DateTime))
            {
                var result = new HisQueryResult<DateTime>(valueCount);
                ReadAllValue(id, startTime, endTime, result);
                return result as HisQueryResult<T>;
            }
            return null;
