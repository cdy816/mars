...
namespace DBInStudio.Desktop.ViewModel
{
    ...
    /// 
    /// </summary>
    public class TagClassDetailViewModel : ViewModelBase, IModeSwitch
    {

        #region ... Variables  ...
        private ComplextTagClass mGroupModel;
        ...
        public bool IsMonitMode
        {
            ...
            set
            {
                if (mIsMonitMode != value)
                {
                    ...
                    if(value)
                    {
                        grid.Columns[grid.Columns.Count-1].Visibility = Visibility.Visible;
                        grid.Columns[grid.Columns.Count - 2].Visibility = Visibility.Visible;
                    }
                    else
                    {
                        grid.Columns[grid.Columns.Count - 1].Visibility = Visibility.Collapsed;
                        grid.Columns[grid.Columns.Count - 2].Visibility = Visibility.Collapsed;
                    }
        ...
        public string FilterDriver
        {
            ...
            set
            {
                if (mFilterDriver != value)
                {
                    ...
                    {
                        RegistorList = TagViewModel.Drivers[value].Item1;
                    }
                    else
                    {
                        RegistorList = null;
        ...
        /// 
        /// </summary>
        public ComplextTagClass GroupModel
        {
            get
            {
                return mGroupModel;
            ...
            Task.Run(() =>
            {
                ...
                foreach (var vv in ltmp)
                {
                    ...
                    else
                    {
                        vv.IsNew = false;
                        vv.IsChanged = false;
                        vv.CheckAvaiableNameDelegate = CheckAvaiableTagName;
                    }
        ...
        private void ContinueQueryTags()
        {
            ...
            if (mTotalPageNumber <0)
            {
                Application.Current.Dispatcher.Invoke(new Action(() => {
                    ...
                    {
                        vv.CheckAvaiableNameDelegate = null;
                    }
              ...
              //  var vtags = new System.Collections.ObjectModel.ObservableCollection<TagViewModel>();
                ...
                var vv = DevelopServiceHelper.Helper.QueryTagByClass(this.GroupModel.Database, this.GroupModel.FullName,mCurrentPageIndex, out mTotalPageNumber,out count,mFilters);
                if (vv != null)
                {
                    foreach (var vvv in vv)
                    {
                        TagViewModel model = new TagViewModel(vvv.Value.Item1, vvv.Value.Item2) { Database = GroupModel.Database, CheckAvaiableNameDelegate = CheckAvaiableTagName,UpdateDelegate=UpdateTag, CanChangedTypeDelegate = CanChangedType,QueryTagSubTagsDelegate= QuerySubTags,IsClassDefine=true };
                        Application.Current?.Dispatcher.Invoke(new Action(() => {
            ...
            else
            {
                if (mTotalPageNumber > mCurrentPageIndex+1)
                {
                    ...
                    var vv = DevelopServiceHelper.Helper.QueryTagByClass(this.GroupModel.Database, this.GroupModel.FullName, mCurrentPageIndex, out totalcount,out count, mFilters);
                    if (vv != null)
                    {
                        foreach (var vvv in vv)
                        {
        ...
        private void RemoveTag()
        {
            ...
            if (CurrentSelectTag != null)
            {
                ...
                {
                    if (DevelopServiceHelper.Helper.RemoveClassTag(GroupModel.Database,GroupModel.Name, vvv.RealTagMode.Id))
                    {
                        SelectGroupTags.Remove(CurrentSelectTag);
                        icount++;
                        mIsModified = true;
                    }
                    vvv.CheckAvaiableNameDelegate = null;
                    vvv.UpdateDelegate = null;
                    vvv.CanChangedTypeDelegate = null;
                    vvv.QueryTagSubTagsDelegate = null;
                }


                if(icount==0)
                {
                    if (DevelopServiceHelper.Helper.RemoveClassTag(GroupModel.Database, GroupModel.Name, CurrentSelectTag.RealTagMode.Id))
                    {
                        SelectGroupTags.Remove(CurrentSelectTag);
                        CurrentSelectTag.CheckAvaiableNameDelegate = null; ;
                        CurrentSelectTag.UpdateDelegate = null;
                        CurrentSelectTag.CanChangedTypeDelegate = null;
                        CurrentSelectTag.QueryTagSubTagsDelegate = null;
                        mIsModified = true;
                        icount++;
                    }
                }
            ...
            else
            {
                ...
                {
                    var vvt = vv as TagViewModel;
                    if (DevelopServiceHelper.Helper.RemoveClassTag(GroupModel.Database,GroupModel.Name, vvt.RealTagMode.Id))
                    {
                        SelectGroupTags.Remove(vvt);
                        vvt.CheckAvaiableNameDelegate = null;
                        vvt.UpdateDelegate = null;
                        vvt.CanChangedTypeDelegate = null;
                        CurrentSelectTag.QueryTagSubTagsDelegate = null;
                        mIsModified = true;
                        icount++;
                    }
        ...
        private void PasteTag()
        {
            if(mCopyTags.Count>0)
            {
                ...
                foreach(var vv in mCopyTags)
                {
                    ...
                    vtag.IsNew = true;
                    vtag.CheckAvaiableNameDelegate = CheckAvaiableTagName;
                    vtag.UpdateDelegate = UpdateTag;
                    vtag.CanChangedTypeDelegate = CanChangedType;
                    vtag.QueryTagSubTagsDelegate = QuerySubTags;
                    vtag.IsClassDefine = true;
                    if (UpdateTag(vtag))
                    {
        ...
        private void NewTag()
        {
            ...
            else
            {
                ...
                //if (this.GroupModel != null && GroupModel is ComplextTagClass)
                //{
                //    tag.Group = (GroupModel as ComplextTagClass).FullName;
                //}
                var vtag = new TagViewModel(tag, null) { IsNew = true,Database=GroupModel.Database,CheckAvaiableNameDelegate= CheckAvaiableTagName,UpdateDelegate=this.UpdateTag,CanChangedTypeDelegate=CanChangedType,QueryTagSubTagsDelegate= QuerySubTags, IsClassDefine = true };
                if (UpdateTag(vtag))
                {
                    this.SelectGroupTags.Add(vtag);
                    CurrentSelectTag = vtag;
        ...
        /// <returns></returns>
        public bool CheckAvaiableTagName(string name)
        {
            return !name.Contains(".") && !DevelopServiceHelper.Helper.CheckClassTagNameExits(this.GroupModel.Database, this.GroupModel.mName, name);
        }
        ...
        {
           return DevelopServiceHelper.Helper.GetAvaiableClassTagName(GroupModel.Database, this.GroupModel.FullName, baseName);
        }
        ...
        /// <param name="name"></param>
        /// <returns></returns>
        public bool CanChangedType(TagType typ, string name)
        {
            return this.GroupModel.Name != name;
        }
        ...
        /// <returns></returns>
        public List<TagViewModel> QuerySubTags(string cls)
        {
            List<TagViewModel> re = new List<TagViewModel>();
            var res = DevelopServiceHelper.Helper.QueryTagByClass(this.GroupModel.Database, cls);
            foreach (var vv in res)
            {
                re.Add(new TagViewModel(vv.Value.Item1, vv.Value.Item2) { QueryTagSubTagsDelegate = QuerySubTags, CheckAvaiableNameDelegate = CheckAvaiableTagName, UpdateDelegate = this.UpdateTag, CanChangedTypeDelegate = CanChangedType});
            }
            return re;
        }
        ...
        private bool UpdateTag(TagViewModel tagmodel)
        {
            ...
            {
                int id;
                var re = DevelopServiceHelper.Helper.AddClassTag(GroupModel.Database,GroupModel.Name, new Tuple<Cdy.Tag.Tagbase, Cdy.Tag.HisTag>(tagmodel.RealTagMode, tagmodel.HisTagMode), out id);
                if (re)
                {
            ...
            else
            {
                ...
                {
                    var re = DevelopServiceHelper.Helper.UpdateClassTag(GroupModel.Database,GroupModel.Name, new Tuple<Cdy.Tag.Tagbase, Cdy.Tag.HisTag>(tagmodel.RealTagMode, tagmodel.HisTagMode));
                    if (re)
                    {
                        tagmodel.IsChanged = false;
                        mIsModified = true;
        ...
        public void DeActive()
        {
            ...
            if(mIsModified)
            {
                DevelopServiceHelper.Helper.UpdateLinkedClassTags(GroupModel.Database, GroupModel.Name);
            }
        }
