...
*/
    ...
    /// 
    /// </summary>
    public class DataFileInfo6: IDataFile
    {
        ...
        private void ReadHeadInfo()
        {
            using (var ss = DataFileSeriserManager.manager.GetDefaultFileSersie())
            {
                ...
                if(!ss.OpenForReadOnly(MetaFileName))
                {
                    return;
                }
        ...
        /// <param name="stime"></param>
        /// <param name="etime"></param>
        /// <param name="filterTimers"></param>
        /// <returns></returns>
        public List<long> ReadDataBlock(int id, DateTime stime, DateTime etime,IEnumerable<DateTime> filterTimers)
        {
            Scan();
            List<long> re = new List<long>();
            var blocks = ReadAddress(id);
            ...
            List<int> filterblocks = new List<int>();
            
            if (filterTimers != null)
            {
                int lpre = -1;
                foreach (var vv in filterTimers)
                {
                    var indx = (int)((vv - StartTime).TotalMinutes / BlockDuration);
                    if (lpre != indx)
                    {
                        filterblocks.Add(indx);
                        lpre = indx;
                    }
                }
            }
            ...
            if (blocks != null && blocks.Length > 0)
            {
                ...
                do
                {
                    if (filterblocks.Count > 0 && !filterblocks.Contains(sindx))
                    {
                        sindx++;
                        ss = ss.AddMinutes(BlockDuration);
                        continue;
        ...
        /// </summary>
        /// <returns></returns>
        private bool IsInTmpDirectoryFile()
        {
            string spath = System.IO.Path.Combine(StartTime.Day.ToString(), "tmp");
            return this.FileName.Contains(spath) || this.MetaFileName.Contains(spath);
        }
                ...
                lock (mLocker)
                {
                    if (IsZipFile && (System.IO.Path.GetExtension(FileName) == DataFileManager.ZipDataFile2Extends || !System.IO.File.Exists(FileName)))
                    {
                        ...
                        if (!System.IO.File.Exists(sspath))
                            DataFileInfo6Extend.UnZipFile(FileName, sspath);
                        ...
                        if (smatafile.EndsWith(DataFileManager.ZipDataMetaFile2Extends))
                        {
                            ...
                            if (!System.IO.File.Exists(smpath))
                                DataFileInfo6Extend.UnZipFile(smatafile, smpath);
                        }
                        else if (smatafile.EndsWith(DataFileManager.DataMetaFile2Extends))
                        {
                            var smpath = System.IO.Path.Combine(spath, System.IO.Path.GetFileName(smatafile).Replace(DataFileManager.ZipDataMetaFile2Extends, DataFileManager.DataMetaFile2Extends));
                            if (!System.IO.File.Exists(smpath) && System.IO.File.Exists(smatafile))
                    ...
                    else if (IsInTmpDirectoryFile())
                    {
                        //检查临时解压文件是否存在
                        string spath = new System.IO.DirectoryInfo(System.IO.Path.GetDirectoryName(FileName)).Parent.FullName;
                        
                        if (!System.IO.File.Exists(this.FileName))
                        {
                            string sname = System.IO.Path.Combine(spath, System.IO.Path.GetFileNameWithoutExtension(FileName))+ DataFileManager.ZipDataFile2Extends;
                            if(System.IO.File.Exists(sname))
                            {
                                DataFileInfo6Extend.UnZipFile(sname, this.FileName);
                            }
                        }

                        if (!System.IO.File.Exists(this.MetaFileName))
                        {
                            string sname = System.IO.Path.Combine(spath, System.IO.Path.GetFileNameWithoutExtension(MetaFileName)) + DataFileManager.ZipDataMetaFile2Extends;
                            if (System.IO.File.Exists(sname))
                            {
                                DataFileInfo6Extend.UnZipFile(sname, this.MetaFileName);
                            }
                        }
                    }
                }
        ...
        public override bool Equals(object obj)
        {
            ...
            {
               return this.FileName == (obj as DataFileInfo6).FileName;
            }
        }
    ...
    public static class DataFileInfo6Extend
    {
        ...
        /// <param name="file"></param>
        /// <returns></returns>
        public static DataFileSeriserbase GetFileSeriser(this DataFileInfo6 file)
        {
            //判断是否为压缩文件
            file.CheckZipFile();
        ...
        public static void ReadAllValue<T>(this DataFileInfo6 file, int tid, DateTime startTime, DateTime endTime, HisQueryResult<T> result)
        {
                ...
                using (QueryContextBase ctx = new QueryContextBase())
                {
                    ...
                    try
                    {
                        ...
                        foreach (var block in blocks)
                        {
                            ...
                            {
                                //从每个数据块的读取值
                                DeCompressOneTimeUnitDataBlockAllValue(vss, block, startTime, endTime, file.TimeTick, result,ctx);
                            }
                        }
                    }
                    catch
        ...
        public static void ReadAllValue<T>(this DataFileInfo6 file,int tid, DateTime startTime, DateTime endTime, HisQueryResult<T> result, ExpressFilter filter, QueryContextBase ctx)
        {
            using (var vss = file.GetFileSeriser())
            {
                if (filter != null)
                {
                    ...
                    {
                        ctx.Add("Filter", filter);
                    }
                }
                IEnumerable<DateTime> tim=null;
                if (ctx.ContainsKey("TimesFilter"))
                {
                    tim = ctx["TimesFilter"] as IEnumerable<DateTime>;
                }

                var blocks = file.ReadDataBlock(tid, startTime, endTime,tim);
                try
                {
                    if (!vss.IsOpened()) return;
        ...
        public static void ModifyHisData<T>(this DataFileInfo6 file, int id,DateTime startime,DateTime endtime, HisQueryResult<T> values)
        {
            using (var vss = file.GetFileSeriser())
            {
        ...
        /// </summary>
        /// <param name="file"></param>
        public static void UpdateToZipFile(this DataFileInfo6 file)
        {
        ...
        public static  void ZipFile(string sfile,string targetfile)
        {
            try
            {
                ...
                sw.Stop();
                LoggerService.Service.Info("DataFileInfo6", "Zip 压缩文件 " + tfile + " 耗时:" + sw.ElapsedMilliseconds);
            }
            catch (Exception ex)
            {
                LoggerService.Service.Erro("DataFileInfo6", "ZipFile: " + ex.Message);
            }
        }
        ...
        /// <param name="id"></param>
        /// <param name="values"></param>
        public static void DeleteHisData<T>(this DataFileInfo6 file, int id, DateTime starttime,DateTime endtime)
        {
            using (var vss = file.GetFileSeriser())
            {
                ...
                var blocks = file.ReadDataBlock(id, list);
                foreach (var block in blocks)
                {
                    var stime = file.StartTime.AddMinutes(file.BlockDuration * block.Key.Item1);
                    var etime = file.StartTime.AddMinutes(file.BlockDuration * (block.Key.Item1 + 1));

                    DeleteBlockHisData<T>(file, vss, id, block.Key.Item2, file.TimeTick, stime, etime, list);
                }
            }
            ServiceLocator.Locator.Resolve<IHisDataManagerService>()?.ClearTmpFile();
            if (file.IsZipFile)
        ...
        /// <param name="times"></param>
        /// <param name="values"></param>
        public static void ModifyBlockHisData<T>(DataFileInfo6 dfile,DataFileSeriserbase file,int id,long address,int timeTick,DateTime blockStartTime,DateTime blockEndTime,List<DateTime> times, HisQueryResult<T> values)
        {
            HisQueryResult<T> result = new HisQueryResult<T>(60*5*1000);
            using (QueryContextBase ctx = new QueryContextBase())
            {
                var mtmp = ReadAllRelativeBlock(file, address,ctx);
                DateTime stime = blockStartTime;
                foreach (var block in mtmp)
                {
                    DeCompressDataBlockAllValue<T>(block, stime, blockEndTime, timeTick, result, ctx);
        ...
        /// <param name="times"></param>
        /// <param name="values"></param>
        public static void ModifyNewBlockHisData<T>(DataFileInfo6 dfile, DataFileSeriserbase file, int id, DateTime blockStartTime, DateTime blockEndTime, List<DateTime> times, HisQueryResult<T> values)
        {
            DateTime sstime = times[0];
            DateTime estime = times[times.Count - 1];
            SortedDictionary<DateTime, Tuple<T, byte>> values2 = new SortedDictionary<DateTime, Tuple<T, byte>>();
        ...
        /// <param name="times"></param>
        /// <param name="values"></param>
        public static void DeleteBlockHisData<T>(DataFileInfo6 dfile, DataFileSeriserbase file, int id, long address, int timeTick, DateTime blockStartTime, DateTime blockEndTime, List<DateTime> timers)
        {
            HisQueryResult<T> result = new HisQueryResult<T>(60 * 5 * 1000);
            using (QueryContextBase ctx = new QueryContextBase())
            {
        ...
        /// <param name="timeTick"></param>
        /// <param name="result"></param>
        private static void DeCompressOneTimeUnitDataBlockAllValue<T>(DataFileSeriserbase file, long address, DateTime startTime, DateTime endTime, int timeTick, HisQueryResult<T> result,QueryContextBase ctx)
        {
            var mtmp = ReadAllRelativeBlock(file,address, ctx);
            DateTime stime = startTime;
            foreach (var block in mtmp)
            {
                DeCompressDataBlockAllValue<T>(block, stime, endTime,timeTick,result, ctx);
                stime = result.LastTime.AddMilliseconds(1);
                block.Dispose();
            }
        }
        ...
        /// <returns></returns>
        private static List<int> DeCompressTimers(byte[] timerVals, int count)
        {
        ...
        /// <returns></returns>
        public static Dictionary<int, DateTime> GetTimers(DataFileSeriserbase source)
        {

            var dtmp = source.ReadBytes(source.CurrentPostion,20);
           
             DateTime sTime = MemoryHelper.ReadDateTime(dtmp, 0);
            int timeTick = MemoryHelper.ReadInt32(dtmp, 8);

            Dictionary<int, DateTime> re = new Dictionary<int, DateTime>();
            var count = MemoryHelper.ReadInt32(dtmp, 12);
            var datasize = MemoryHelper.ReadInt32(dtmp, 16);
            byte[] datas = source.ReadBytes(source.CurrentPostion,datasize);
        ...
        private static void CheckFillDateTime(DataFileSeriserbase file,long curpos,MarshalMemoryBlock block, QueryContextBase ctx)
        {
            long pos = file.CurrentPostion;
            //long opos = block.Position;
            var vltmp = block.ReadULong(1);
            block.Position = 0;
            if (vltmp> 0x8000000000000000)
            {
                if (!ctx.ContainsKey(vltmp.ToString()))
                {
                    file.GoTo(curpos - (long)(vltmp ^ 0x8000000000000000) +10);
                    ctx.Add(vltmp.ToString(), GetTimers(file));
                }
            }
            file.GoTo(pos);
        }
        ...
        /// <param name="address"></param>
        /// <returns></returns>
        private static IEnumerable<MarshalMemoryBlock> ReadAllRelativeBlock(DataFileSeriserbase file,long address, QueryContextBase ctx)
        {
            ...
            {
                file.GoTo(next + 5);
                var vsize = file.ReadInt(next + 5);
                mblock = new MarshalMemoryBlock(vsize,vsize);
                file.GoTo(next + 9);
                mblock.ReadFromStream(file.GetStream(), (int)mblock.AllocSize);
                mtmp.Add(mblock);

                CheckFillDateTime(file, next, mblock, ctx);
        ...
        /// <param name="timeTick"></param>
        /// <param name="result"></param>
        private static void DeCompressDataBlockAllValue<T>(MarshalMemoryBlock memory, DateTime startTime, DateTime endTime, int timeTick, HisQueryResult<T> result,QueryContextBase ctx)
        {
            ...
            if (tp != null)
            {
                ...
                {
                    tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, result, ctx);
                }
                else
                {
                    DateTime time;
                    ...
                    switch (tpp)
                    {
                        case TagType.Bool:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, htmp, ctx);
                        ...
                        case TagType.Byte:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, btmp, ctx);
                        ...
                        case TagType.Short:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, stmp, ctx);
                        ...
                        case TagType.UShort:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, ustmp, ctx);
                        ...
                        case TagType.Int:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, itmp, ctx);
                        ...
                        case TagType.UInt:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, uitmp, ctx);
                        ...
                        case TagType.Long:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, ltmp, ctx);
                        ...
                        case TagType.ULong:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, ultmp, ctx);
                        ...
                        case TagType.DateTime:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, dttmp, ctx);
                        ...
                        case TagType.Float:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, ftmp, ctx);
                        ...
                        case TagType.Double:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, dtmp, ctx);
                        ...
                        case TagType.String:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, sstmp, ctx);
                        ...
                        case TagType.IntPoint:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, iptmp, ctx);
                        ...
                        case TagType.IntPoint3:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, ip3tmp, ctx);
                        ...
                        case TagType.UIntPoint:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, uptmp, ctx);
                        ...
                        case TagType.UIntPoint3:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, uip3tmp, ctx);
                        ...
                        case TagType.LongPoint:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, liptmp, ctx);
                        ...
                        case TagType.ULongPoint:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, uliptmp, ctx);
                        ...
                        case TagType.LongPoint3:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, lip3tmp, ctx);
                        ...
                        case TagType.ULongPoint3:
                            ...
                            tp.DeCompressAllValue(memory, 1, startTime, endTime, timeTick, ulip3tmp, ctx);
        ...
        /// <returns></returns>
        public static object Read<T>(this DataFileInfo6 file, int tid, DateTime time, QueryValueMatchType type)
        {
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public static HisQueryResult<T> Read<T>(this DataFileInfo6 file, int tid, List<DateTime> times, QueryValueMatchType type)
        {
        ...
        private static long GetHeadPointer(DataFileInfo6 file, int id,int bindex, QueryContext context)
        {
            var vb = context.GetHeadPoint(file.FileName, bindex);
            if (vb < 0)
            {
                context.RegisorHeadPoint(file.FileName, file.ReadAddress(id));
                return context.GetHeadPoint(file.FileName, bindex);
            }
            return vb;
        }
        ...
        ///// <returns></returns>
        //public static object ReadLastAvaiableValue<T>(this DataFileInfo6 file, DataFileSeriserbase datafile, int tid,int dindex, QueryContext context)
        //{
        //    object oval=null;
        //    do
        //    {
        ...
        /// <param name="dindex"></param>
        /// <returns></returns>
        public static object ReadLastAvaiableValue2<T>(this DataFileInfo6 file, DataFileSeriserbase datafile, int tid, int dindex, QueryContext context,out bool needCancel)
        {
            object oval = null;
            //oval = context.GetBlockLastValue(file.FileName, dindex);
            //if (oval != null)
            ...
            var mmp = ReadAllRelativeBlock(datafile, GetHeadPointer(file, tid, dindex, context),context);
        ...
        private static string GetPreDataFile(string currentFile, int id, DateTime time)
        {
            ...
            //string ssfile = currentFile.LastIndexOf(".") > 0 ? currentFile.Substring(0, currentFile.LastIndexOf(".")) : currentFile;

            //string Database = ssfile.Substring(0, ssfile.Length - 15);

            string dbname = System.IO.Path.GetFileNameWithoutExtension(currentFile);
            if (dbname.Length < 15)
                return String.Empty;
            
            dbname = dbname.Substring(0, dbname.Length - 15);

            var sindex = currentFile.IndexOf(System.IO.Path.Combine(((int)(id / 100000)).ToString("X3"), time.Year.ToString(), time.Month.ToString(), time.Day.ToString()));
            if (sindex < 0) 
                return String.Empty;

            string filepath = currentFile.Substring(0, sindex);

            //string Database = ssfile.Substring(0, ssfile.Length-15);

            string Database = System.IO.Path.Combine(filepath, ((int)(id / 100000)).ToString("X3"), "{0}", "{4}", "{5}", dbname);
            ...
            string sfile = ListPreviewDataFiles3(time, tmp,tmp2);
            DateTime dtime = time;
            while (string.IsNullOrEmpty(sfile))
            {
                dtime = dtime.AddHours(-4);
                sfile = ListPreviewDataFiles3(dtime, tmp, tmp2);
                if (!string.IsNullOrEmpty(sfile) || (time-dtime).TotalDays > 365)
                {
                    break;
                }
            }
        ...
        private static string ListPreviewDataFiles3(DateTime time, string filetemplate, string filetemplate2)
        {
            ...
            if (ind > 0)
            {
                sfile = string.Format(filetemplate, stime.Year, stime.Month.ToString("D2"), stime.Day.ToString("D2"), (ind - 1).ToString("D2"), time.Month.ToString(), time.Day.ToString());
                ...
                {
                    sfile = string.Format(filetemplate2, stime.Year, stime.Month.ToString("D2"), stime.Day.ToString("D2"), (ind - 1).ToString("D2"), time.Month.ToString(), time.Day.ToString());
                    if (ifileservice.CheckHisFileExist(sfile, out realfile))
                    {
                        return realfile;
                    }
            ...
            else
            {
                ...
                sfile = string.Format(filetemplate, stime.Year, stime.Month.ToString("D2"), stime.Day.ToString("D2"), ind.ToString("D2"), stime.Month.ToString(), stime.Day.ToString());
                if (ifileservice.CheckHisFileExist(sfile, out realfile))
                {
                    return realfile;
                }
                ...
                {
                    sfile = string.Format(filetemplate2, stime.Year, stime.Month.ToString("D2"), stime.Day.ToString("D2"), ind.ToString("D2"), stime.Month.ToString(), stime.Day.ToString());
                    if (ifileservice.CheckHisFileExist(sfile, out realfile))
                    {
                        return realfile;
                    }
        ...
        {
            // string ssfile = System.IO.Path.Combine( System.IO.Path.GetDirectoryName(currentFile), System.IO.Path.GetFileNameWithoutExtension(currentFile));
            ...
            string tmp = Database + ((int)(id / 100000)).ToString("X3") + "{0}{1}{2}04{3}.dbd2";
            string tmp2 = Database + ((int)(id / 100000)).ToString("X3") + "{0}{1}{2}04{3}.zdbd2";
            string sfile =  ListNextDataFiles3(time,tmp,tmp2);
            DateTime dtime = time;
            DateTime dnow = DateTime.Now;
            while(string.IsNullOrEmpty(sfile))
            {
                dtime = dtime.AddHours(4);
                sfile = ListNextDataFiles3(dtime, tmp, tmp2);
                if(!string.IsNullOrEmpty(sfile) || (dtime-time).TotalDays>365 || (dtime - dnow).TotalHours>4)
                {
                    break;
                }
            }
        ...
        private static string ListNextDataFiles3(DateTime time, string filetemplate,string filetemplate2)
        {
            ...
            if (ind < 5)
            {
                sfile = string.Format(filetemplate, time.Year, time.Month.ToString("D2"), time.Day.ToString("D2"), (ind + 1).ToString("D2"), time.Month.ToString(), time.Day.ToString());
                if (ifileservice.CheckHisFileExist(sfile, out realfile))
                {
                    return realfile;
                }
                ...
                {
                    sfile = string.Format(filetemplate2, time.Year, time.Month.ToString("D2"), time.Day.ToString("D2"), (ind + 1).ToString("D2"), time.Month.ToString(), time.Day.ToString());
                    if(ifileservice.CheckHisFileExist(sfile,out realfile))
                    {
                        return realfile;
                    }
        ...
        ///// <param name="file"></param>
        ///// <returns></returns>
        //private static IEnumerable<string> ListPreviewDataFiles(DataFileInfo6 file)
        //{
        //    var ifileservice = ServiceLocator.Locator.Resolve<IDataFileService>();
        ...
        //private static IEnumerable<string> ListNextDataFiles(DataFileInfo6 file)
        //{
        //    var ifileservice = ServiceLocator.Locator.Resolve<IDataFileService>();
        //    string sname = System.IO.Path.GetFileName(file.FileName);
        ...
        ///// <param name="index"></param>;
        ///// <returns></returns>
        //public static object ReadLastAvaiableValueCrossFile<T>(this DataFileInfo6 currentfile,DataFileSeriserbase datafile,int id,int index, QueryContext context)
        //{
        ...
        //        {
        //            if (obj != null) break;
        //            DataFileInfo6 dfile = new DataFileInfo6() { FileName = vv,IsZipFile = vv.EndsWith(".zdbd2")};
        ...
        /// <param name="index"></param>;
        /// <returns></returns>
        public static object ReadPreFileLastAvaiableValue<T>(this DataFileInfo6 currentfile, int id, QueryContext context)
        {
            object obj = null;
            DataFileInfo6 dfile = currentfile;
            DateTime dstarttime = dfile.StartTime;
            while (obj == null)
            {
                string sfile = GetPreDataFile(dfile.FileName, id, dstarttime);
                if (!string.IsNullOrEmpty(sfile))
                {
                    var stime = DataFileManager.ParseFileToTime(sfile, out int oid, out int hhspan);
                    dfile = new DataFileInfo6() { FileName = sfile, IsZipFile = sfile.EndsWith(".zdbd2"),StartTime=stime };
                    using (var dsf = dfile.GetFileSeriser())
                    {
                        for (int i = 47; i > -1; i--)
                        {
        ...
        /// <returns></returns>
        public static object ReadFileLastAvaiableValue<T>(this DataFileInfo6 currentfile, int id, QueryContext context)
        {
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public static object ReadFileLastAvaiableValue<T>(this DataFileInfo6 currentfile,int blockIndex, int id, QueryContext context)
        {
            using (var dsf = currentfile.GetFileSeriser())
            {
        ...
        ///// <param name="index"></param>
        ///// <returns></returns>
        //public static object ReadFirstAvaiableValueCrossFile<T>(this DataFileInfo6 currentfile,DataFileSeriserbase datafile,int id,int index,QueryContext context)
        //{
        ...
        /// <param name="index"></param>;
        /// <returns></returns>
        public static object ReadNextFileFirstAvaiableValue<T>(this DataFileInfo6 currentfile, int id, QueryContext context)
        {
            object obj = null;
            DataFileInfo6 dfile = currentfile;
            ...
            while (obj == null)
            {
                string sfile = GetNextDataFile(dfile.FileName, id, dstarttime);
                if (!string.IsNullOrEmpty(sfile))
                {
                    var stime = DataFileManager.ParseFileToTime(sfile, out int oid, out int hhspan);
                    dfile = new DataFileInfo6() { FileName = sfile, IsZipFile = sfile.EndsWith(".zdbd2"), StartTime = stime };
                    using (var dsf = dfile.GetFileSeriser())
                    {
                        for (int i = 0; i < 48; i++)
                        {
                            obj = dfile.ReadFirstAvaiableValue2<T>(dsf, id, i, context, out bool needcancel);
                            if (obj != null || needcancel)
                            {
                                break;
                            }
                ...
                {
                    //一直读到最后内存中的第一个数据
                    var mMemoryService = ServiceLocator.Locator.Resolve<IHisQueryFromMemory>() as IHisQueryFromMemory;
                    if (mMemoryService != null)
                    {
                        var vobj = mMemoryService.GetStartValue<T>((long)id, out DateTime time, out byte qua);
                        if (vobj != null)
                            obj = new TagHisValue<T>() { Value = (T)vobj, Quality = qua, Time = time };
                    }
                    break;
                }
            }
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public static object ReadFileFirstAvaiableValue<T>(this DataFileInfo6 currentFile, int id, QueryContext context)
        {
            using (var dsf = currentFile.GetFileSeriser())
            {
                for (int i = 0; i < 48; i++)
                {
                    var val = currentFile.ReadFirstAvaiableValue2<T>(dsf, id, i, context, out bool needcancel);
                    if(val!=null)
                    {
                        return val;
                    }
        ...
        /// <param name="currentFile"></param>
        /// <param name="id"></param>
        ///  <param name="blockindex">起始搜索的BlockID</param>
        /// <param name="context"></param>
        /// <returns></returns>
        public static object ReadFileFirstAvaiableValue<T>(this DataFileInfo6 currentFile, int id,int blockindex, QueryContext context)
        {
            using (var dsf = currentFile.GetFileSeriser())
            {
                for (int i = blockindex; i < 48; i++)
                {
                    var val = currentFile.ReadFirstAvaiableValue2<T>(dsf, id, i, context, out bool needcancel);
                    if (val != null)
                    {
        ...
        ///// <param name="dindex"></param>
        ///// <returns></returns>
        //public static object ReadFirstAvaiableValue<T>(this DataFileInfo6 file, DataFileSeriserbase datafile, int tid, int dindex,QueryContext context)
        //{
        //    object oval = null;
        //    do
        //    {
        ...
        public static object ReadFirstAvaiableValue2<T>(this DataFileInfo6 file, DataFileSeriserbase datafile, int tid, int dindex, QueryContext context, out bool needCancel)
        {
            object oval = null;
            oval = context.GetBlockFirstValue(file.FileName, dindex);
            if (oval != null)
        ...
        /// <param name="type"></param>
        /// <param name="result"></param>
        public static void Read<T>(this DataFileInfo6 file, int tid, List<DateTime> times, QueryValueMatchType type, HisQueryResult<T> result,QueryContext context)
        {
            using (var vff = file.GetFileSeriser())
            {
                ...
                object mLastReadPreObj = null;
                object mLastReadNextObj = null;
                bool mHasPreLast=false;
                bool mHasNextLast=false;
                ...
                foreach(var block in blocks)
                {
                    ...
                    else
                    {
                        ...
                        {
                            object preval = ReadFileLastAvaiableValue<T>(file,(int)(block.Key.Item1 - 1), tid, context);
                            if(preval==null)
                            {
                                if (!mHasPreLast)
                                {
                                    preval = ReadPreFileLastAvaiableValue<T>(file, tid, context);
                                    mHasPreLast = true;
                                    mLastReadPreObj=preval;
                                }
                                else
                                {
                                    preval = mLastReadPreObj;
                                }
                            }
                            ...
                            object nxtval = ReadFileFirstAvaiableValue<T>(file,tid, block.Key.Item1 + 1, context);
                            if(nxtval==null)
                            {
                                if(!mHasNextLast)
                                {
                                    nxtval = ReadNextFileFirstAvaiableValue<T>(file, tid, context);
                                    mHasNextLast = true;
                                    mLastReadNextObj = nxtval;
                                }
                                else
                                {
                                    nxtval = mLastReadNextObj;
                                }
                            ...
                            switch (type)
                            {
                                case QueryValueMatchType.Previous:
                                    ...
                                    if (pval.HasValue)
                                    {
                                        ...
                                        foreach (var vv in block.Value)
                                        {
                                            result.Add(pval.Value.Value, vv, squa);
                                        }
                                    }
                                    else
                                    {
                                ...
                                case QueryValueMatchType.After:
                                    if (nval.HasValue)
                                    {
                                        ...
                                        foreach (var vv in block.Value)
                                        {
                                            result.Add(nval.Value.Value, vv, squa);
                                        }
                                    }
                                    else
                                    {
                                ...
                                case QueryValueMatchType.Linear:
                                    if(pval.HasValue && nval.HasValue)
                                    {
                                        ...
                                        if (typeof(T) == typeof(bool) || typeof(T) == typeof(string) || typeof(T) == typeof(DateTime))
                                        {
                                            ...
                                            foreach (var vv in block.Value)
                                            {
                                                if ((vv - pval.Value.Time) > (nval.Value.Time - vv))
                                                {
                                                    result.Add(nval.Value.Value, vv, equa);
                                                }
                                                else
                                                {
                                                    result.Add(pval.Value.Value, vv, squa);
                                        ...
                                        else
                                        {
                                            ...
                                            foreach (var vv in block.Value)
                                            {
                                                var pval1 = (vv - pval.Value.Time).TotalMilliseconds;
                                                var tval1 = (nval.Value.Time - pval.Value.Time).TotalMilliseconds;
                                                var vtmp = Convert.ToDouble(sval2) - Convert.ToDouble(sval1);
                                                double val1 = 0;
                                                if (vtmp == 0)
                                                {
                                                ...
                                                {
                                                    val1 = pval1 / tval1 * (vtmp) + Convert.ToDouble(sval1);
                                                }
                                                if (pval1 <= 0)
                                                {
                                                    //说明数据有异常，则取第一个值
                                                    result.Add((object)sval1, vv, pval.Value.Quality);
                                                }
                                                else
                                                {
                                                    result.Add((object)val1, vv, (pval1 / tval1) < 0.5 ? squa : equa);
                                                }
                                            }
                                        }
                                    }
                ...
                //sw.Stop();
                //Debug.WriteLine(file.FileName+ ":"+ sw.ElapsedMilliseconds);
            }
        }
        ...
        /// <param name="qa"></param>
        /// <returns></returns>
        public static bool IsBadQuality(byte qa, QueryContext ctx)
        {
        ...
        public static void Read<T>(this DataFileInfo6 file, DataFileSeriserbase datafile, int tid, long address, int index, List<DateTime> dataTimes, QueryValueMatchType type, HisQueryResult<T> res, QueryContext context)
        {
            //Dictionary<string,object> contenxt = new Dictionary<string,object>();
            context["hasnext"]= false;
            var mtmp = ReadAllRelativeBlock(datafile, address, context);
            context["block"] = mtmp;
            context["index"] = 0;
                ...
                if (temp.Count > 0)
                {
                    DeCompressDataBlockValue<T>(block, temp, file.TimeTick, type, res, new Func<byte,QueryContext, object>((tp,ctx) =>
                    {
                        ...
                        if (tp == 0)
                        {
                            ...
                            else if(context.IgnorCloseQuality)
                            {
                                //读取上一个Block 值的第一个值
                                object vobj = file.ReadFileLastAvaiableValue<T>(dindex - 1,tid, context);
                                if (vobj != null)
                                {
                                    return vobj;
                                }
                                //读取上一个文件的最后一个值
                                vobj = file.ReadPreFileLastAvaiableValue<T>(tid, ctx);
                                if (vobj != null)
                                {
                                    return vobj;
                                }
                        ...
                        else
                        {
                            ...
                            {
                                //读取下一个Block 值的第一个值
                                object vobj = file.ReadFileFirstAvaiableValue<T>(tid, dindex + 1, context);
                                if (vobj != null)
                                {
                                    return vobj;
                                }
                                //读取下一个文件的第一个值
                                vobj = file.ReadNextFileFirstAvaiableValue<T>(tid, ctx);
                                if (vobj != null)
                                {
                                    return vobj;
                                }
        ...
        private static object DeCompressDataBlockRawValue<T>(IEnumerable<MarshalMemoryBlock> memory, byte readValueType, out bool needCancel, QueryContext context, string file, int blockid)
        {
            ...
            qc.IgnorCloseQuality = context.IgnorCloseQuality;
            foreach (var vv in memory)
            {
                ...
                context.HasExitedQuality = qc.HasExitedQuality;
                if (!context.IgnorCloseQuality && re.IsMin())
                {
                    needCancel = true;
                    return null;
                }
