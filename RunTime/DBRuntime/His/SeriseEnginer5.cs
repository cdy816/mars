...
//  Version 1.0
//  种道洋
//  较SeriseEnginer5 修改数据块指针为：固定的100000个变量的，单个变量每5分钟一个数据块指针的结构
//==============================================================
using System;
...
namespace Cdy.Tag
{
    ...
    /// 序列话引擎
    /// </summary>
    public class SeriseEnginer5 : IDataSerialize3, IDisposable
    {
        ...
        /// 
        /// </summary>
        private Dictionary<int, SeriseFileItem5> mSeriserFiles = new Dictionary<int, SeriseFileItem5>();
        ...
        public void Init()
        {
            ...
            SeriseFileItem5 mcurrentItem=null;
            ...
            foreach (var vv in histag)
            {
                ...
                {
                    mcurrentItem = new SeriseFileItem5() { FileDuration = FileDuration, BlockDuration = BlockDuration, TagCountOneFile = TagCountOneFile, DatabaseName = DatabaseName, Id = did, StatisticsMemory = mStatisticsMemory };
        ...
        public void CheckAndAddSeriseFile(IEnumerable<int> tagIds)
        {
            lock (mSeriserFiles)
            {
                foreach (var tagId in tagIds)
                {
                    ...
                    if (!mSeriserFiles.ContainsKey(did))
                    {
                        var sf = new SeriseFileItem5() { FileDuration = FileDuration, BlockDuration = BlockDuration, TagCountOneFile = TagCountOneFile, DatabaseName = DatabaseName, Id = did, StatisticsMemory = mStatisticsMemory };
        ...
        public void Start()
        {
            LoggerService.Service.Info("SeriseEnginer5", "开始启动");
            mIsClosed = false;
        ...
        public void Stop()
        {
            LoggerService.Service.Info("SeriseEnginer5", "开始停止");
            mIsClosed = true;
            resetEvent.Set();
            closedEvent.WaitOne();
            resetEvent.Dispose();
        ...
        private void ThreadPro()
        {
            ...
            while (!mIsClosed)
            {
               ...
               mIsBusy = true;
                ...
                sw.Start();
                LoggerService.Service.Info("SeriseEnginer5", "********开始执行存储********", ConsoleColor.Cyan);
                //#endif
                ...
                //#if DEBUG
                sw.Stop();
                LoggerService.Service.Info("SeriseEnginer5", ">>>>>>>>>完成执行存储>>>>>>>  ElapsedMilliseconds:" + sw.ElapsedMilliseconds, ConsoleColor.Cyan);
            ...
            while (!mIsClosed)
            {
                ...
                if ((DateTime.Now - mLastDataFileReArrangeProcessTime).TotalSeconds > 60)
                {
                    if (System.IO.Directory.Exists(wpath))
                    {
                        ...
                        {
                            LoggerService.Service.Erro("SeriseEnginer5", "DataFileReArrangeThreadPro: " + ex.Message);
                        }
                    }
        ...
        private void ZipFile(string sfile)
        {
            try
            {
                ...
                sw.Stop();
                LoggerService.Service.Info("SeriseEnginer5", "Zip 压缩文件 " +tfile +" 耗时:"+sw.ElapsedMilliseconds);
            }
            catch(Exception ex)
            {
                LoggerService.Service.Erro("SeriseEnginer5", "ZipFile: " + ex.Message);
            }
        }
                ...
                if (time.Day != mLastBackupDay)
                {
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(backpath))
                        {
                            ...
                            {
                                LoggerService.Service.Warn("SeriseEnginer5", "free disk space is lower in backup path");
                            }
                        }
                        ...
                        if (System.IO.Directory.Exists(wpath))
                        {
                            foreach (var vv in new System.IO.DirectoryInfo(wpath).GetFiles("*.his"))
                            {
                                    ...
                                    if ((time - vv.LastWriteTime).TotalDays >= HisDataKeepTimeInPrimaryPath)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(backpath))
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                LoggerService.Service.Erro("SeriseEnginer5", "There is not enough space for backup! free size:" + (GetDriverFreeSize(backpath) / 1024.0 / 1024) + "M. required size:" + (vv.Length / 1024.0 / 1024) + " M");
                                            }
                                        }
                                        else
                                        {
                                ...
                                {
                                    LoggerService.Service.Erro("SeriseEnginer5", ex.Message + " " + ex.StackTrace);
                                }
                            }
                        }
                    }
    ...
    /// 
    /// </summary>
    public class SeriseFileItem5 : IDisposable
    {
        #region ... Variables  ...
        ...
        private byte[] GeneratorDataRegionHeader(DateTime mCurrentTime, out int totallenght, out int datalen)
        {
            ...
            int len = GetDataRegionHeaderLength() + MaxTagId * (blockcount * 8);

            totallenght = len;

            if (mCurrentTime == DateTime.MinValue)
            {
                LoggerService.Service.Erro("SeriseEnginer5", $"时间值无效! { new StackTrace().ToString() } ");
            }
            ...
            using (System.IO.MemoryStream mHeadMemory = new System.IO.MemoryStream(bvals))
            {
                ...
                mHeadMemory.Write(BitConverter.GetBytes(MaxTagId));
        ...
        {
            return System.IO.Path.Combine(SeriseEnginer5.HisDataPath, GetFileName(time));
        }
        ...
        {
            return System.IO.Path.Combine(SeriseEnginer5.HisDataPath, GetStatisticsFileName(time));
        }
        ...
        private Dictionary<int, List<long>> GetDataRegionHeadPoint(string sfile, SortedDictionary<int,List<DateTime>> ids, DateTime time, out DataFileSeriserbase mFileReader)
        {
            ...
            else
            {
                ...
                {
                    dfs = mFileWriter2;
                    string hisfile = System.IO.Path.Combine(SeriseEnginer5.HisDataPath, sfile.Replace(DataFileExtends, HisDataFileExtends));
                    ...
                    {
                        var vfile = System.IO.Path.Combine(SeriseEnginer5.HisDataPath, sfile);
                        if (mFileWriter2.CreatOrOpenFile(vfile))
                        {
                            ...
                            LoggerService.Service.Info("SeriseEnginer5", "new file head and data region head.");
            ...
            if (isuserhisfile)
            {
                ...
                foreach (var vv in ids)
                {
                    foreach (var vvv in vv.Value)
                    {
                        ...
                        if (ltmp < 0)
                        {
                            LoggerService.Service.Warn("SeriseEnginer5", "不允许修改本次运行之前时间的历史记录!");
                            return re;
                        }
            ...
            else
            {
                foreach (var vv in ids)
                {
                    foreach (var vvv in vv.Value)
                    {
                        ...
                        ltmp += mDataRegionHeadSize + MaxTagId * 8 * bid + icount * 8;
                        if (re.ContainsKey(vv.Key))
                        {
                            re[vv.Key].Add(ltmp);
                        }
        ...
        public unsafe void FreshManualDataToDisk()
        {
            ...
            while(mManualHisDataCach.Count>0)
            {
                ...
                LoggerService.Service.Info("SeriseEnginer5", "SeriseFileItem " + this.Id + " 完成存储,数据块:" + vv.Value.Count + " 数据量:" + datasize + " 耗时:" + sw.ElapsedMilliseconds);
        ...
        /// 
        /// </summary>
        public SeriseFileItem5 Init()
        {
            if (mHeadMemory != null)
            {
                if(MaxTagId*8!=mHeadMemory.AllocSize)
                {
                    mHeadMemory.Dispose();
                    mHeadMemory = new MarshalMemoryBlock(MaxTagId * 8, MaxTagId * 8);
                }
            }
            else
            {
            ...
            if (!CheckInSameFile(time))
            {
                ...
                else
                {
                    ...
                    else 
                    {
                        ...
                        AppendDataRegionHeader(time, mFileWriter, mPreDataRegion);
        ...
        public void SaveToFile(MarshalMemoryBlock mProcessMemory, long dataOffset, DateTime time, DateTime endTime)
        {
            ...
            try
            {
                lock (mFileLocker)
                {
                    ...
                    //计算出本次更新的头地址地址
                    var pointAddr = mBlockPointOffset + MaxTagId * 8 * bid;
